...
\subsection{Cifrario a Shift}
    Per $K=3$, questo tipo di cifrario è anche detto \textit{Cifrario di Cesare}.
    ...
    \begin{definition}[Cifrario a sostituzione] 
        Sia $M = C = \mathbb{Z}_{26}$. $\mathcal{K}$ è composto da tutte le possibili permutazioni dei 26 simboli $0,1, \dots, 25$.\\
        $\forall \pi \in \mathcal{K}$ definiamo
        \[ e_{\pi}(x) = \pi(x) \]
        \[ d_{\pi}(y) = \pi^{-1}(y)\]
    ...
    È possibile decifrare il messaggio (trovando la chiave) attraverso l'analisi delle frequenze delle lettere presenti nel \textit{ciphertext}. L'analisi delle 10 lettere più frequenti nel testo cifrato permette l'abbinamento con le lettere corrispondenti.
    ...
    \subsection{Cifrario affine}
        ...
        \begin{definition}[Cifrario Affine]
            Sia $M = C = \mathbb{Z}_{26}$ e $\mathcal{K} = \{ (a,b) \in \mathbb{Z}_{26} \times \mathbb{Z}_{26} \mid \gcd(a,26) = 1 \}$                
    ...
    \begin{definition}[Cifrario di Vigenère]
        Sia $m \in \mathbb{Z}$, $M = C = \mathcal{K} = (\mathbb{Z}_{26})^{m}$. Per una chiave $K = (k_{1},\dots,k_{m})$, si definiscono per $x = (x_1,\dots,x_m) \in M$ e $y = (y_1,\dots,y_m) \in C$
        \begin{align*}
...
\begin{definition}[Cifrario di Hill]
    Sia $m \geq 2 \in \mathbb{Z}$, $M = C = (\mathbb{Z})^{26}$ e $\mathcal{K} = \{ \text{matrici invertibili } m \times m\}$, si definiscono 
    \[
